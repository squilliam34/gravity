...
def load_embedding_cache(year: str) -> pd.DataFrame:
    """
    Load existing embedding cache.
    """
    embedding_cache = EMBEDDING_CACHE_DIR / year / 'gemini_item1_raw_cache.parquet'
...
def save_embedding_cache(cache: pd.DataFrame, year:str):
    """
...
def retrieve_item1_batch(cik_dict: dict, year:str) -> pd.DataFrame:
    """
    ...
    year (str): The year to fetch the 10-k for.
    ...
    item1_cache = DESCRIPTION_CACHE_DIR / year / 'item1_cache.parquet'  
...
def embed_text(descriptions: pd.DataFrame, year:str) -> np.ndarray:
    """
    ...
    cache = load_embedding_cache(year)
    ...
    if len(missing) > 0:
        ...
        save_embedding_cache(cache, year)
...
def get_semantic_distances(tickers: list[str], year:str) -> pd.DataFrame:
    """
    ...
    descriptions = retrieve_item1_batch(ciks, year)

    # Embed descriptions
    print('Embedding descriptions...')
    result = embed_text(descriptions, year)
    embeddings = result[0]
