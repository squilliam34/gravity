...
[^4]: $\cos \theta_{ij} = \frac{\vec{v}_i \cdot \vec{v}_j}{\lVert \vec{v}_i \rVert \lVert \vec{v}_j \rVert}$
...
[^6]: $D_{ij} = \sqrt{(\beta_i - \beta_j)^T \Sigma^{-1} (\beta_i - \beta_j)}$